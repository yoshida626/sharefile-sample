...
void load_platform(const sg4::Engine&)
{
  ...
  /* yoshdia added codes from here*/
  /* create PFS */
  auto* PFS_zone = sg4::create_full_zone("PFS_zone")->set_parent(root);
  const sg4::Host* host = PFS_zone->create_host("PFS", "1Gf");
  PFS_zone->seal();
  /* connect cluster2 and PFS from here*/
  const sg4::Link* l_read = root->create_link("PFS_read", "100Gbps")->set_latency("100us");
  const sg4::Link* l_write = root->create_link("PFS_write", "100Gbps")->set_latency("100us");
  sg4::LinkInRoute link_read{l_read};
  sg4::LinkInRoute link_write{l_write};
  root->add_route(PFS_zone, right_cluster, {link_read}, false);
  root->add_route(right_cluster, PFS_zone, {link_write}, false);
  /* yoshida added codes to here */
